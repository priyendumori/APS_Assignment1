...
int leftChildren(avl_node* node){
    //cout<<"1 "<<endl;
    if(node==NULL || node->left == NULL && node->right == NULL)
        return 1;

    //cout<<"2 "<<endl;
    return leftChildren(node->left) + leftChildren(node->right);
}

avl_node* findKthMin(avl_node* node, int k){
    
    while(node!=NULL){
        //cout<<"k "<<k<<" lc "<< (leftChildren(node)) <<endl;
        if( k == leftChildren(node)){
            break;
        }
        else if(k > leftChildren(node) - 1){
            k = k - (leftChildren(node));
            node=node->right;
        }
        else{
            node=node->left;
...
int main(){
    ...
    while(m--){
        ...
        switch(q){
            ...
            case 3: int kmin = findKthMin(root, x)->data ;
                    cout<<kmin<<endl;
                    deleteNode(root, kmin);
                    display(root,1,root);
                    break;
        }
    }
